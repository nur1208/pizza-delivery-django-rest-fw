...
class UserCreationSerializer(serializers.ModelSerializer):
    ...
    def validate(self,attrs):
        ...
        if email:
            raise ValidationError(detail="User with email exists",
                code=status.HTTP_403_FORBIDDEN)

        username=User.objects.filter(username=attrs.get('username')).exists()

        if username:
            raise ValidationError(detail="User with username exists",
                code=status.HTTP_403_FORBIDDEN)

        phone_number=User.objects.filter(phone_number=attrs.get('phone_number')).exists()

        if phone_number:
            raise ValidationError(detail="User with phone number exists",
                code=status.HTTP_403_FORBIDDEN)
